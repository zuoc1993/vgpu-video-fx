...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu/packages/effect-core/src/effects/handheld-cam/effect.wgsl
struct _vgsl_6fdae404__Params {
  time: f32,
  intensity: f32,
  speed: f32,
  zoom: f32,
...
@group(0) @binding(2) var<uniform> params: _vgsl_6fdae404__Params;
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  var p = _vgsl_35d1d59a__containUv(uv, params.resolution, params.videoSize);
  p = _vgsl_35d1d59a__zoomAt(p, breath, vec2f(0.5) + drift);
  p = _vgsl_35d1d59a__rotateUv(p, angle, vec2f(0.5));

  let vel = vec2f(cos(t * 1.6), sin(t * 1.15) * 0.65) * params.blur * params.intensity * (0.012 + 0.07 * whip);
  var color = _vgsl_35d1d59a__directionalBlur(src, samp, p, vel);
  var bloom = vec3f(0.0);
  let radius = 0.006 * params.glow;
  for (var i = 0; i < 8; i += 1) {
    let a = f32(i) * 0.785398;
    bloom += max(_vgsl_35d1d59a__sampleVideo(src, samp, p + vec2f(cos(a), sin(a)) * radius).rgb - vec3f(0.62), vec3f(0.0));
  }
...
fn _vgsl_35d1d59a__rotateUv(uv: vec2f, angle: f32, center: vec2f) -> vec2f {
  let c = cos(angle);
  let s = sin(angle);
  let p = uv - center;
...
fn _vgsl_35d1d59a__directionalBlur(src: texture_2d<f32>, samp: sampler, uv: vec2f, dir: vec2f) -> vec3f {
  var acc = vec3f(0.0);
  for (var i = 0; i < 9; i += 1) {
    let k = (f32(i) / 8.0 - 0.5) * 2.0;
    acc += _vgsl_35d1d59a__sampleVideo(src, samp, uv + dir * k).rgb;
  }
  return acc / 9.0;
}
