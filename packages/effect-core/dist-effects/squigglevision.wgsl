...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu/packages/effect-core/src/effects/squigglevision/effect.wgsl
struct _vgsl_3454f39a__Params {
  time: f32,
  strength: f32,
  fps: f32,
  scale: f32,
...
@group(0) @binding(2) var<uniform> params: _vgsl_3454f39a__Params;
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let n = _vgsl_95d6fc5a__vnoise2(v * params.scale + vec2f(tick * 0.113, tick * 0.071));
  return _vgsl_35d1d59a__sampleVideo(src, samp, v + n * params.strength * 0.02);
}
...
fn _vgsl_95d6fc5a__cornerNoise(cell: vec2i) -> vec2f {
  let q = _vgsl_9a0b5690__pcg2d(bitcast<vec2u>(cell));
  return vec2f(_vgsl_9a0b5690__unitFloat(q.x), _vgsl_9a0b5690__unitFloat(q.y)) * 2.0 - 1.0;
}

fn _vgsl_95d6fc5a__vnoise2(p: vec2f) -> vec2f {
  let i = floor(p);
  let f = p - i;
  let u = f * f * (3.0 - 2.0 * f);
  let ic = vec2i(i);
  let a = _vgsl_95d6fc5a__cornerNoise(ic);
  let b = _vgsl_95d6fc5a__cornerNoise(ic + vec2i(1, 0));
  let c = _vgsl_95d6fc5a__cornerNoise(ic + vec2i(0, 1));
  let d = _vgsl_95d6fc5a__cornerNoise(ic + vec2i(1, 1));
