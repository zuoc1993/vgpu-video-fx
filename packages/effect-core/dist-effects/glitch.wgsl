...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu-video-fx/packages/effect-core/src/effects/glitch/effect.wgsl
struct _vgsl_62b3bd65__Params {
  time: f32,
  intensity: f32,
  speed: f32,
  slices: f32,
...
@group(0) @binding(2) var<uniform> params: _vgsl_62b3bd65__Params;

fn _vgsl_62b3bd65__sampleGlitch(src: texture_2d<f32>, samp: sampler, uv: vec2f) -> vec4f {
...
fn _vgsl_62b3bd65__glitchTint(h: vec2f) -> vec3f {
  let i = u32(h.x * 6.0);
...
fn _vgsl_62b3bd65__wash(base: vec3f, tint: vec3f, amt: f32) -> vec3f {
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let burst = _vgsl_aa51502b__hash2(vec2f(tick, 0.7)).x;
  ...
  let sliceN = _vgsl_aa51502b__hash2(vec2f(row, tick));
  vuv.x += (sliceN.x - 0.5) * 0.42 * on;
  vuv.x += (_vgsl_aa51502b__hash2(vec2f(floor(vuv.y * 240.0), tick)).x - 0.5) * 0.08 * on;

  let blockN = _vgsl_aa51502b__hash2(floor(vuv * vec2f(14.0, 8.0)) + vec2f(tick, 4.2));
  if (blockN.x > 1.0 - params.block * 0.22 * on) {
    vuv += (blockN - vec2f(0.5)) * 0.12 * on;
  }

  let split = params.rgbSplit * on * 0.05;
  let r = _vgsl_62b3bd65__sampleGlitch(src, samp, vuv + vec2f(split, 0.0)).r;
  let g = _vgsl_62b3bd65__sampleGlitch(src, samp, vuv + vec2f(-split * 0.35, split * 0.15)).g;
  let b = _vgsl_62b3bd65__sampleGlitch(src, samp, vuv - vec2f(split, 0.0)).b;
  ...
  let colH = _vgsl_aa51502b__hash2(vec2f(col + 0.37, tick));
  color = _vgsl_62b3bd65__wash(color, _vgsl_62b3bd65__glitchTint(colH), cover);

  let rectH = _vgsl_aa51502b__hash2(vec2f(tick, 8.1));
  let rx = rectH.x * 0.4;
  let ry = rectH.y * 0.35;
  if (vuv.x > rx && vuv.x < rx + 0.28 + rectH.x * 0.3 && vuv.y > ry && vuv.y < ry + 0.2 + rectH.y * 0.25) {
    color = _vgsl_62b3bd65__wash(color, _vgsl_62b3bd65__glitchTint(rectH + vec2f(0.2, 0.1)), cover * 0.85);
  }

  let edge = _vgsl_aa51502b__hash2(vec2f(tick, 2.2));
  if (vuv.x > 0.86 && edge.x > 0.35) {
    color = _vgsl_62b3bd65__wash(color, vec3f(0.2, 0.05, 0.78), cover * 0.7);
  }

  let line = floor(vuv.y * 96.0);
  let lineH = _vgsl_aa51502b__hash2(vec2f(line, tick + 5.0));
  if (lineH.x > 0.93) {
    color = _vgsl_62b3bd65__wash(color, _vgsl_62b3bd65__glitchTint(lineH), cover * 0.9);
  }
