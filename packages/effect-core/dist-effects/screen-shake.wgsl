...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu/packages/effect-core/src/effects/screen-shake/effect.wgsl
struct _vgsl_d3cc8a87__Params {
  time: f32,
  intensity: f32,
  speed: f32,
  punch: f32,
...
@group(0) @binding(2) var<uniform> params: _vgsl_d3cc8a87__Params;

@fragment
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let cell = floor(params.time * params.speed * 20.0);
  let n = _vgsl_9a0b5690__hash2(vec2f(cell, 3.3)) - vec2f(0.5);
  ...
  vuv = _vgsl_35d1d59a__zoomAt(vuv + offset, zoom, vec2f(0.5));
  let dir = normalize(vec2f(0.85, -0.55) + n * 0.35) * params.blur * params.intensity * (0.02 + 0.09 * hit);
  return vec4f(_vgsl_35d1d59a__directionalBlur(src, samp, vuv, dir), 1.0);
}
