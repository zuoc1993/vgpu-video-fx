...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu-video-fx/packages/effect-core/src/effects/curtain-wind/effect.wgsl
struct _vgsl_de5708b0__Params {
  time: f32,
  strength: f32,
  folds: f32,
  speed: f32,
...
@group(0) @binding(2) var<uniform> params: _vgsl_de5708b0__Params;

const _vgsl_de5708b0__TAU: f32 = 6.2831853;
// 光源：左上方、略偏向观察者。
const _vgsl_de5708b0__LIGHT: vec3f = vec3f(-0.3505, -0.4506, 0.8211);
// 暖色背光（阳光透布）。
const _vgsl_de5708b0__WARM: vec3f = vec3f(1.0, 0.82, 0.58);

// 阵风包络 [0,1]：几条慢速正弦叠加后锐化，制造起风/停歇的呼吸感。
fn _vgsl_de5708b0__gustEnv(t: f32) -> f32 {
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let env = (1.0 - params.gust) * 0.62 + params.gust * _vgsl_de5708b0__gustEnv(t) * 1.25;

  // ---- 布面高度场 z(v) --------------------------------------------------
  // 垂直褶皱为主，相位沿 y 向下漂移 → 风从杆边一路滚到下摆；再加整面布
  // 的低频"呼吸"鼓起。
  let wob = v.y * 2.6 - t * 1.8;
  let phase = v.x * params.folds * _vgsl_de5708b0__TAU
            + sin(wob) * 0.9 * freedom
  ...
  let dzdx = amp * cos(phase) * params.folds * _vgsl_de5708b0__TAU
           + bamp * cos(bphase) * 1.2;
  ...
  let col = _vgsl_17688d6e__sampleVideo(src, samp, suv);

  // ---- 光照 -------------------------------------------------------------
  let n = normalize(vec3f(-dzdx, -dzdy, 1.0));
  let diff = max(dot(n, _vgsl_de5708b0__LIGHT), 0.0);
  var shade = 0.55 + 0.48 * diff;                 // 褶皱上流动的明暗
  shade *= 1.0 + z * 1.4;                         // 鼓向镜头的布面更亮
  let rim = (1.0 - n.z) * (1.0 - n.z);            // 侧对的褶边透背光
  let hv = normalize(_vgsl_de5708b0__LIGHT + vec3f(0.0, 0.0, 1.0));
  let sheen = pow(max(dot(n, hv), 0.0), 24.0) * 0.10;   // 丝绸光泽
  let glow = _vgsl_de5708b0__WARM * (rim * 0.30 + sheen) * params.bleed * inside;
