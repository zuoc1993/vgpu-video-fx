...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  vuv = _vgsl_17688d6e__zoomAt(vuv + fromCenter * radial * params.distortion * 0.45 * pulse, zoom, center);
  ...
  for (var i = 0; i < 10; i += 1) {
    ...
    let r = _vgsl_17688d6e__sampleVideo(src, samp, tap + vec2f(split, 0.0)).r;
    let g = _vgsl_17688d6e__sampleVideo(src, samp, tap).g;
    let b = _vgsl_17688d6e__sampleVideo(src, samp, tap - vec2f(split, 0.0)).b;
    color += vec3f(r, g, b);
  }
  color /= 10.0;
