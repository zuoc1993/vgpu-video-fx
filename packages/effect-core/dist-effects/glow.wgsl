...
// vgsl-module: /Users/zuoc/Documents/vscode/vgpu/packages/effect-core/src/effects/glow/effect.wgsl
struct _vgsl_19ca728e__Params {
  blur: f32,
  amount: f32,
  threshold: f32,
  resolution: vec2f,
...
@group(0) @binding(2) var<uniform> params: _vgsl_19ca728e__Params;

const _vgsl_19ca728e__DIR = array<vec2f, 8>(
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let base = _vgsl_35d1d59a__sampleVideo(src, samp, v).rgb;
  let r = params.blur / params.resolution.x;
  var acc = vec3f(0.0);
  for (var i = 0u; i < 8u; i += 1u) {
    acc += _vgsl_35d1d59a__sampleVideo(src, samp, v + _vgsl_19ca728e__DIR[i] * r).rgb;
  }
  acc = acc / 8.0;
