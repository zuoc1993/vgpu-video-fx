...
fn hash2(p: vec2f) -> f32 {
  return hash(dot(p, vec2f(127.1, 311.7)));
}
...
fn barrel(uv: vec2f, curvature: f32) -> vec2f {
  var p = uv * 2.0 - 1.0;
  let r2 = dot(p, p);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  var sampleUv = barrel(v, params.curvature);

  // Row-band shifts (glitch), driven by time.
  let band = floor(px.y / 6.0);
  let shouldShift = step(1.0 - gi * 0.65, hash(band * 7.3 + seed * 100.0));
  let shiftAmount = (hash(band + seed * 31.0) - 0.5) * 0.09 * gi;
  sampleUv.x += shiftAmount * shouldShift;

  // Chromatic aberration radial, extra split on glitch rows.
  let uvCenter = sampleUv - 0.5;
  let lineHash = hash(floor(px.y) * 1.3 + seed * 200.0);
  ...
  var col = vec4f(r, g, b, 1.0) * inB(sampleUv);

  // Film grain.
  let pixelHash = hash2(floor(px) + fract(seed * 500.0) * 999.0);
